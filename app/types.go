...
type User struct {
	...
	UUID         string     `gorm:"column:uid;unique_index" sql:"default:''" json:"-"`
	...
	OpenID       string     `sql:"default:''" json:"-"`
	PassWord     string     `gorm:"column:password" sql:"not null" json:"password,omitempty"`
	PassWordType int        `gorm:"column:password_type" sql:"default:0"` //0表示默认 1表示只用md5
	Salt         string     `gorm:"column:salt" sql:"default:''" json:"-"`
	Area         string     `gorm:"column:area" sql:"default:''" json:"area,omitempty"`
	LoginType    uint       `gorm:"column:login_type" json:"-"` //1账号 2微信
	InvitedCode  string     `gorm:"column:invited_code;index" sql:"default:''" json:"invited_code"`
	ModifyPWD    bool       `gorm:"column:modify_pwd"`
